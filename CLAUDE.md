...
```bash
python run.py                      # 开发环境（debug模式，端口5001）
...
- 用户表 `user`：id, username, password, can_approve, permission_level
- 登录：`POST /api/auth/login`
- 当前用户：`GET /api/auth/current_user` → `{id, username, can_approve, permission_level}`
...
| 级别 | 名称 | 权限 |
|------|------|------|
| 1 | 查看 | 只能查看数据，无增删改保存权限 |
| 2 | 编辑 | 增删改保存权限，无审核权限 |
| 3 | 管理 | 所有权限（增删改保存+审核） |

权限存储在 `session['permission_level']`，后端通过 `session.get('permission_level', 0)` 检查。
...
用于截图、调试页面：
```bash
playwright-cli open http://localhost:5001/login --browser=chrome --persistent  # 打开浏览器
playwright-cli screenshot  # 截图
playwright-cli snapshot     # 获取页面元素快照
```

截图文件保存在当前目录的 `.playwright-cli/` 下，调试产物统一放在 `debug/` 目录。
...
`out_order_print.html` 的 `.info-row` 和 `.info-row-2` 边框样式要统一：
- `.info-row` 用 `border: 1px solid #000; border-bottom: none;`
- `.info-row-2` 用 `border-left/right/top: 1px solid #000;`，最后一行用内联样式 `style="border-bottom: 1px solid #000;"`
- 避免用 `:last-child` 或 `:last-of-type` 选择器（因页面结构问题容易失效）
